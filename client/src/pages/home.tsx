...
export default function Home() {
  ...
  // WebSocket spot prices (fallback to REST if missing)
  ...
  // REST tickers (fallback values + 10s refresh)
  ...
  // Portfolio (15s refresh, only if logged in)
  ...
  // Mirror REST changes into local change objects
  useEffect(() => {
  ...
  // Gainers → compute **top 3** display with 24h %
  const top3Gainers = useMemo(() => {
    if (!Array.isArray(gainers)) return null;
    const items = gainers
      .map((g) => {
        const rawSym = g.symbol || g.pair || g.ticker || "";
        const symbol = String(rawSym).toUpperCase();
        const pct =
          (typeof g.change24h === "string" || typeof g.change24h === "number"
            ? Number(g.change24h)
            : undefined) ??
          (typeof g.priceChangePercent === "string" || typeof g.priceChangePercent === "number"
            ? Number(g.priceChangePercent)
            : undefined);

        if (!symbol || !Number.isFinite(pct as number)) return null;
        return { symbol, pct: Number(pct) };
      })
  ...
  const gainersDisplay = useMemo(() => {
    if (!top3Gainers || top3Gainers.length === 0) return "—";
    return top3Gainers
      .map((g) => `${g.symbol} ${g.pct >= 0 ? "+" : ""}${nf2.format(g.pct)}%`)
      .join(" · ");
  }, [top3Gainers]);
  ...
  return (
    <div className="flex-1 overflow-hidden">
      <div className="p-6">
        ...
        {/* Dashboard Overview (unchanged visuals) */}
