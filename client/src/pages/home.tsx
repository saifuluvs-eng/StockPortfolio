 ...
 * - Set Vercel env var VITE_API_BASE to your backend, e.g. https://api.yourdomain.com
 * - WebSocket /ws is blocked on Vercel for this app, so we fall back to REST-only.
 */
...
// Build full API URL. If API_BASE is "", we keep relative (local dev).
...
// ---------- Query Functions (use API_BASE) ----------
async function qPortfolioSummary(): Promise<{
  ...
  // 2) Fallback: compute from positions
...
async function qHighPotentialCount(): Promise<{
  count: number | null;
  ts: number;
}> {
  // Try POST like your page
  const post = await safeJson<any>(apiUrl("/api/scanner/high-potential"), {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({}),
  });
  if (post && Array.isArray(post.results))
    return { count: post.results.length, ts: Date.now() };
  ...
  if (obj && Array.isArray(obj.results))
    return { count: obj.results.length, ts: Date.now() };

  return { count: null, ts: Date.now() };
}

async function qGainersTop3(): Promise<{
  top: { symbol: string; pct: number }[] | null;
  ts: number;
}> {
...
export default function Home() {
  ...
  // ---------- Lower "Market Overview" (REST fallback; WS disabled on Vercel) ----------
  ...
  const { data: btcTicker } = useQuery({
    ...
    enabled: !!apiUrl("/api/market/ticker/BTCUSDT"),
  });
  ...
  const { data: ethTicker } = useQuery({
    ...
    refetchInterval: 10_000,
    enabled: !!apiUrl("/api/market/ticker/ETHUSDT"),
  });

  useEffect(() => {
    if (btcTicker?.priceChangePercent != null) setBtcChange(btcTicker);
    if (btcTicker?.price != null)
      setPrices((p) => ({ ...p, BTCUSDT: Number(btcTicker.price) || p.BTCUSDT }));
  }, [btcTicker]);

  useEffect(() => {
    if (ethTicker?.priceChangePercent != null) setEthChange(ethTicker);
    if (ethTicker?.price != null)
      setPrices((p) => ({ ...p, ETHUSDT: Number(ethTicker.price) || p.ETHUSDT }));
  }, [ethTicker]);

  // WebSocket only for non-Vercel hosts
  useEffect(() => {
    if (isVercel) return; // blocked on Vercel
  ...
  const { data: hp } = useQuery({
    ...
    enabled: !!API_BASE || !isVercel, // on Vercel require API_BASE
  });
  ...
  const { data: watchCount } = useQuery({
    ...
    refetchInterval: 300_000,
    enabled: (!!user && (!!API_BASE || !isVercel)),
  });
  ...
  const portfolioValueDisplay =
    port?.totalValue == null ? "$0.00" : `$${nf2.format(port.totalValue)}`;
  const portfolioPctDisplay =
    port?.totalPnlPercent == null
      ? "+0.00%"
      : `${port.totalPnlPercent >= 0 ? "+" : ""}${nf2.format(
          port.totalPnlPercent
        )}%`;
  ...
  const watchDisplay =
    watchCount == null || Number.isNaN(watchCount) ? "—" : nf0.format(watchCount);
  const aiDisplay =
    aiCount == null || Number.isNaN(aiCount) ? "—" : nf0.format(aiCount);
              ...
              <CardContent className="p-6 h-full flex flex-col justify-between">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-lg font-bold text-foreground mt-2">
                      {hp?.count == null ? "—" : nf0.format(hp.count)}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      Active signals
                    </p>
                    <p className="text-[10px] text-muted-foreground mt-1">
                      Last updated {hp?.ts ? new Date(hp.ts).toLocaleTimeString() : "—"}
                    </p>
                  </div>
              ...
              <CardContent className="p-6 h-full flex flex-col justify-between">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-[10px] text-muted-foreground mt-1">
                      Last updated{" "}
                      {gain?.ts ? new Date(gain.ts).toLocaleTimeString() : "—"}
                    </p>
                  </div>
              ...
              <CardContent className="p-6 h-full flex flex-col justify-between">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-lg font-bold text-foreground mt-2">
                      {watchCount ? Math.min(watchCount, 3) : 0}
                    </p>
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="text-right">
                  ...
                  >
                    {parseFloat((btcChange as any)?.priceChangePercent || "0") >= 0 ? "+" : ""}
                    {nf2.format(
                      parseFloat((btcChange as any)?.priceChangePercent || "0")
                    )}
                    %
                  </p>
                </div>
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="text-right">
                  ...
                  >
                    {parseFloat((ethChange as any)?.priceChangePercent || "0") >= 0 ? "+" : ""}
                    {nf2.format(
                      parseFloat((ethChange as any)?.priceChangePercent || "0")
                    )}
                    %
                  </p>
                </div>
