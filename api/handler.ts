...
class TechnicalIndicators {
  ...
  async scanMomentum(): Promise<any[]> {
    ...
    try {
      ...
      const scanTimeframe = '1h'; // 1h for better Stop-Loss resolution
      const scanLimit = 400;
...
async function topPicksStrategy(req: any, res: any) {
  try {
...
export default async function handler(req: any, res: any) {
  try {
    ...
    if (seg[0] === "market") {
      ...
      if (seg[1] === "strategies") {
        if (seg[2] === "trend-dip") return trendDipStrategy(req, res);
        if (seg[2] === "volume-spike") return volumeSpikeStrategy(req, res);
        if (seg[2] === "momentum") return momentumStrategy(req, res);
        if (seg[2] === "support-resistance") return supportResistanceStrategy(req, res);
        if (seg[2] === "top-picks") return topPicksStrategy(req, res);

        return bad(res, 404, "Unknown strategy", { path: seg.join("/") });
      }
